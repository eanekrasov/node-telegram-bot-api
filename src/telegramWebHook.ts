import { FatalError, ParseError } from './errors';
...
const debug = Debug('node-telegram-bot-api');
...
export default class {
  ...
  close() {
    ...
    return new Promise((resolve, reject) => {
      this._webServer.close((error:any) => {
   ...
   * @private
   */
  _parseBody(error:any, body:any) {
    if (error) {
      return this._error(new FatalError(error));
    }
    let data;
    try {
      data = JSON.parse(body.toString());
    } catch (parseError) {
      let s = parseError.message;
      let e = new ParseError(s);
   ...
   */
  _requestListener(req:any, res:any) {
